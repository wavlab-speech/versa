...
def process_jsonl_file(input_file, output_dir='filtered_results'):
    ...
    filters = {
        ...
        'wer_lt_0.10': lambda x: x['wer'] < 0.10,
        'wer_lt_0.20': lambda x: x['wer'] < 0.20,
        'wer_lt_0.30': lambda x: x['wer'] < 0.30,
        'insertion_error_lt_0.05': lambda x: x['insertion_error'] < 0.05,
        'insertion_error_lt_0.10': lambda x: x['insertion_error'] < 0.10,
    }
    ...
    if processed_lines:
        ...
        print(f"Average Insertion Error: {sum(insertion_values)/len(insertion_values):.4f}")
        print(f"Average Deletion Error: {sum(deletion_values)/len(deletion_values):.4f}")
        print(f"Average Substitution Error: {sum(substitution_values)/len(substitution_values):.4f}")
