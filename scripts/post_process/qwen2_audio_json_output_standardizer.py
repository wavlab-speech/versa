...
class JsonOutputStandardizer:
    ...
    def _process_llm_output(self, metric_name, llm_output):
        ...
        prefixes_to_remove = [
            "The category is", "Category:", "Answer:", "Output:", 
            "The speaker count is", "Number of speakers:", "The number is",
    ...
    def _rules_based_standardize(self, metric_name, raw_output):
        ...
        if metric_name == "qwen_language":
            ...
            match = re.search(r'(language|spoken) (?:is|in) ([\w\s]+)', translated_output, re.IGNORECASE)
            if match:
    ...
    def standardize(self, metric_name, raw_output):
        ...
        if self.llm_available:
            try:
                ...
                logging.warning(f"LLM processing failed for {metric_name}: {e}. Falling back to rules-based approach.")
...
def process_directory(input_dir, output_dir=None, use_llm=True, file_pattern="*.json"):
    ...
    if output_dir is None:
        output_dir = os.path.join(os.path.dirname(input_dir), f"{os.path.basename(input_dir)}_standardized")
    ...
    if not json_files:
        logging.warning(f"No JSON files found in {input_dir} matching pattern {file_pattern}")
    ...
    for json_file in tqdm(json_files, desc="Processing JSON files"):
        try:
            ...
            output_file = Path(output_dir) / rel_path.with_stem(f"{json_file.stem}_standardized")
...
def main():
    ...
    parser.add_argument("--output", "-o", help="Output file or directory (default: adds '_standardized' to input name)")
    parser.add_argument("--no-llm", action="store_true", help="Use rules-based approach instead of LLM")
