...
def write_vis_csv(input_file, output_file):
    ...
    grouped = (
        df.groupby("category")
        .agg(value=("metric_name", "count"), mean=("mean", "mean"), std=("std", "mean"))
        .reset_index()
    )
...
def main():
    ...
    parser.add_argument(
        "--input_file",
        help="the path of text result csv file",
        default="metrics_analysis.csv",
        type=str,
    )
    parser.add_argument(
        "--output_file",
        help="the new format csv file for visualization",
        default="metrics_tree.csv",
        type=str,
    )
