...
# -------------------------------
# CDPAM Metric Definition and Tests
# -------------------------------
def test_cdpam_metric_identical(fixed_audio):
    """
    When comparing an audio signal with itself, the cdpam distance should be 0.0.
    """
    model = cdpam_model_setup()
    scores = cdpam_metric(model, fixed_audio, fixed_audio, 16000)
    assert (
        scores["cdpam_distance"] == 0.0
    ), f"Expected cdpam distance == 0.0 for identical signals, got {scores['cdpam_distance']}"
